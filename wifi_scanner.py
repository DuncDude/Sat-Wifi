...
class WiFiScanner:
    ...
    def get_highest_signal_bssids(self, output_file):
        ...
        for line in lines:
            ...
            if len(values) >= 14:  # Ensure the line has at least 14 columns
                bssid = values[0]
                # Extract and remove leading/trailing whitespaces
                essid = values[13].strip()
                signal = int(values[8])
    ...
    def save_to_txt(self, highest_bssid_dict, txt_file):
        with open(txt_file, "w") as file:
            ...
            for bssid, (essid, highest_signal) in highest_bssid_dict.items():
                file.write(
                    f"BSSID: {bssid}, ESSID: {essid}, Highest Signal Strength: {highest_signal} dBm\n")
    ...
    def run(self, interface, txt_file):
        ...
        try:
            print(
                f"Starting airodump-ng on interface {monitor_mode_interface}...")
            airodump_process = self.start_airodump(
                monitor_mode_interface, output_file)

            # Run airodump-ng for 30 seconds
            time.sleep(30)

            print(
                "30 seconds have passed. Getting BSSIDs with their highest signal strengths...")
            ...
            print(
                f"Stopping airodump-ng on interface {monitor_mode_interface}...")
